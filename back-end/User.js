...
module.exports = {
                    ...
                    connection.query('INSERT INTO transaction(type, stockID, quantity, price, time, email) VALUES ?', [values], function (err, result, field) {
                        ...
                        connection.query('SELECT * FROM asset WHERE stockID = ? AND email = ?', [stockID, email], function (err, result, field) {
                            ...
                            if (result.length == 0) {
                                ...
                                connection.query('INSERT INTO asset (stockID, buyPrice, quantity, email) VALUES ?', [[[stockID, price, quantity, email]]], function(err, result, field) {
                                    if(err) throw err;
                                });
                            }
                            else
    ...
    sellStock: function(email, stockID, quantity, price){
        return new Promise( function(resolve, reject) {
            ...
            new Promise( function(resolve1, reject1) {
                ...
                var percProfit = ( ( (price - old_buyPrice) ) / old_buyPrice ) * 100;
                var values = [['sell', stockID, quantity, price, __date, email, percProfit]];
                //console.log(percProfit);
                connection.query('INSERT INTO transaction(type, stockID, quantity, price, time, email, percProfit) VALUES ?', [values], function (err, result, field) {
                    if (err) throw err;
                });
                ...
                // }
                // else {
                    var old_sellPrice = Number(result[0].sellPrice);
                    var old_quantity_sell = Number(result[0].quantity_sell);
                    var new_quantity_sell = Number(old_quantity_sell) + Number(quantity);
                    var new_sellPrice = ( (old_sellPrice * old_quantity_sell) + (quantity * price) ) / new_quantity_sell;
                    var new_roi = ( Number( new_sellPrice - old_buyPrice ) / old_buyPrice ) * 100;
                    connection.query('UPDATE asset SET quantity = ? WHERE email = ? AND stockID = ?', [new_quantity, email, stockID], function (err, result, field) {
                        if (err) throw err;
                    });
                    connection.query('UPDATE asset SET quantity_sell = ? WHERE email = ? AND stockID = ?', [new_quantity_sell, email, stockID], function (err, result, field) {
                        if (err) throw err;
                    });
                    connection.query('UPDATE asset SET sellPrice = ? WHERE email = ? AND stockID = ?', [new_sellPrice, email, stockID], function (err, result, field) {
                        if (err) throw err;
                    });
                    connection.query('UPDATE asset SET ROI = ? WHERE email = ? AND stockID = ?', [new_roi, email, stockID], function (err, result, field) {
                        if (err) throw err;
                    });
                // }
                resolve(1);
    ...
    getAssets: function(email) {
        return new Promise( function(resolve, reject) {
            connection.query('SELECT * FROM stock INNER JOIN asset ON stock.stockID = asset.stockID WHERE asset.email = ? ', [email], function(err, result, field) {
                if(err) throw err;
                resolve(result);
            });
        });
